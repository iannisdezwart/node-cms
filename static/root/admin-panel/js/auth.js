...
const setPadlock = (mode) => {
    const padlockImage = $('#padlock > img');
    if (mode == 'locked') {
...
const login = async (secondTry = false) => {
    const popupResult = await popup('Login', secondTry ? 'Incorrect password. ' : '' + 'Please enter your password to login', [
        {
            name: 'Login'
        }
    ], [
